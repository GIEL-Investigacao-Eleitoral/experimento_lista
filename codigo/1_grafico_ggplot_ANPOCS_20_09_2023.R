...
resultado11$categoria = 'global'

library(ggplot2)
ggplot(resultado11, aes(x=1, y=m.diff, group=1)) +
  geom_pointrange(aes(ymin = low, ymax = upp), color='royalblue', fill='royalblue',linewidth = 2)+
...
resultado_sexo1$categoria = c('masculino','feminino')


resultado = resultado11 %>% add_row(resultado_sexo1)
resultado$x = 1:3

ggplot(resultado, aes(x=categoria, y=m.diff, group=1)) +
  geom_pointrange(aes(ymin = low, ymax = upp), color='royalblue', fill='royalblue',linewidth = 2)+
  geom_point(shape=21, size=5, fill='royalblue')+
  geom_hline(yintercept = 0,linetype='dashed', col = 'red') + 
  coord_flip()+
